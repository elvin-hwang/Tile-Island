...
ECS::Entity Blobule::createBlobule(vec2 position, blobuleCol col, std::string colString)
{
    ...
    setTrajectory(entity, 0);
    removeTrajectory(entity);
    setTrajectory(entity, 0);
...
void Blobule::setTrajectory(ECS::Entity entity, float power) {
    ...
    trajMotion.scale = static_cast<vec2>(trajMesh.texture.size);
    trajMotion.position = blobMotion.position + vec2{ cos(blobMotion.angle) * (blobMotion.scale.x / 2 + trajMotion.scale.x / 2), sin(blobMotion.angle)* (blobMotion.scale.x / 2 + trajMotion.scale.x / 2) };
